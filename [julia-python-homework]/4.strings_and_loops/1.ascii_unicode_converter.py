"""Develop an application that can encrypt and decrypt
multiple messages using ASCII and Unicode code points
...
for indexCount in range(1, count + 1):
    ...
    if purpose == "encrypt":
        encrypted_message = []
        for char in message:
            encrypted_message.append(ord(char))
        print(
            f"Encrypted Message {indexCount}: {' '.join(str(x) for x in encrypted_message)}"
        )

    if purpose == "decrypt":
        decrypted_message = []
        for num in message.split():
            decrypted_message.append(chr(int(num)))
        print(f"Decrypted Message {indexCount}: {''.join(decrypted_message)}")
